...
class TblUsers(Base):
    ...
    uuid: Mapped[str] = mapped_column("usr_uuid", String(36), nullable=False, unique=True)
    username: Mapped[str] = mapped_column("usr_username", String(255), nullable=False)
    email: Mapped[str] = mapped_column("usr_email", String(255), nullable=False, unique=True)
    ...
    created_at: Mapped[datetime] = mapped_column("usr_createdAt", DateTime, nullable=False, default=datetime.now())
    updated_at: Mapped[datetime] = mapped_column("usr_updatedAt", TIMESTAMP, nullable=True, server_default=text("NULL ON UPDATE current_timestamp()"))
